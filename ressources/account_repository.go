...
func (r *AccountRepo) Update(accounts []domain.Account, context usecases.QueryContext) ([]domain.Account, error) {
	...
	for i, account := range accounts {
		...
		err := queryCopy.Where("accounts.id = ?", account.ID).First(&oldAccount).Updates(accounts[i]).Error
		if err != nil {
			transaction.Rollback()
...
func (r *AccountRepo) UpdateByID(id int, account *domain.Account,
	...
	err = query.Where("accounts.id = ?", id).First(&oldAccount).Updates(account).Error
	if err != nil {
		if strings.Contains(err.Error(), "constraint") {
			return nil, internalerrors.NewViolatedConstraint(err.Error())
		}
...
func (r *AccountRepo) DeleteByID(id int, context usecases.QueryContext) error {
	...
	err = query.Where("accounts.id = ?", id).First(&account).Delete(domain.Account{}).Error
	if err != nil {
		if strings.Contains(err.Error(), "record not found") {
