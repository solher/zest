...
func (r *RoleRepo) Update(roles []domain.Role, context usecases.QueryContext) ([]domain.Role, error) {
	...
	for i, role := range roles {
		...
		err := queryCopy.Where("roles.id = ?", role.ID).First(&oldRole).Updates(roles[i]).Error
		if err != nil {
			transaction.Rollback()
...
func (r *RoleRepo) UpdateByID(id int, role *domain.Role,
	...
	err = query.Where("roles.id = ?", id).First(&oldRole).Updates(role).Error
	if err != nil {
		if strings.Contains(err.Error(), "constraint") {
			return nil, internalerrors.NewViolatedConstraint(err.Error())
		}
...
func (r *RoleRepo) DeleteByID(id int, context usecases.QueryContext) error {
	...
	err = query.Where("roles.id = ?", id).First(&role).Delete(domain.Role{}).Error
	if err != nil {
		if strings.Contains(err.Error(), "record not found") {
