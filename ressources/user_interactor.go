...
func (i *UserInter) Upsert(users []domain.User, filter *usecases.Filter, ownerRelations []domain.Relation) ([]domain.User, error) {
	...
	for k := range usersToUpdate {
		err = i.AfterUpdate(&users[k])
		if err != nil {
			return nil, err
		}
	}

	for k := range usersToCreate {
		err = i.AfterCreate(&users[k])
		if err != nil {
			return nil, err
		}
