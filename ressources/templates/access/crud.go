...
var routes = &typewriter.Template{
	...
	func set{{.Type}}AccessOptions(routeDir *interfaces.RouteDirectory, controller *{{.Type}}Ctrl) {
  	...
  	routeDir.Add(key.For("DeleteByID"), &interfaces.Route{Method: "DELETE", Path: "/{{.Name}}s/:id", Handler: &deleteByID, Visible: true}, true)
		...
		routeDir.Add(key.For("CreateRelated"), &interfaces.Route{Method: "POST", Path: "/{{.Name}}s/:pk/:related", Handler: &related, Visible: true}, false)
		routeDir.Add(key.For("FindRelated"), &interfaces.Route{Method: "GET", Path: "/{{.Name}}s/:pk/:related", Handler: &related, Visible: true}, false)
		routeDir.Add(key.For("FindByIDRelated"), &interfaces.Route{Method: "GET", Path: "/{{.Name}}s/:pk/:related/:fk", Handler: &relatedOne, Visible: true}, false)
		routeDir.Add(key.For("UpsertRelated"), &interfaces.Route{Method: "PUT", Path: "/{{.Name}}s/:pk/:related", Handler: &related, Visible: true}, false)
		routeDir.Add(key.For("DeleteAllRelated"), &interfaces.Route{Method: "DELETE", Path: "/{{.Name}}s/:pk/:related", Handler: &related, Visible: true}, false)
		routeDir.Add(key.For("DeleteByIDRelated"), &interfaces.Route{Method: "DELETE", Path: "/{{.Name}}s/:pk/:related/:fk", Handler: &relatedOne, Visible: true}, false)
	}
`}
